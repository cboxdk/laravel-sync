...
- **The policy's `view` rule decides which rows a device gets.** It used to be `viewAny` only, so every row in a tenant was synced to everyone who could read the type. A row that becomes hidden is removed from the device.
