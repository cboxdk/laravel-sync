...
- A hidden field is never synced, now also when a model lists it in `$syncFields`. Devices keep the copy they have and their writes to it are refused as not writable; to go on syncing it, take it out of `$hidden` or override `syncFields()`. A model whose listed fields are all hidden is refused at boot rather than syncing nothing.
