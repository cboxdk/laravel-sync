...
trait HandlesSyncRequests
{
    ...
    private function syncRun(\Closure $handler, Request $request): JsonResponse
    {
        ...
        try {
            ...
            //
            // PDOException, not only Laravel's QueryException: the sync store
            // runs on the raw PDO handle, so a lock wait on the space row
            // arrives as a bare PDOException and used to skip this entirely.
            // A dropped connection is the same kind of answer - nothing was
            // committed, and the same mutation may be sent again.
            //
            ...
            if (! $this->causedByConcurrencyError($query) && ! $this->causedByLostConnection($query)) {
                throw $query;
            }
